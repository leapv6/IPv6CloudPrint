...
public:
   ...
   int Fields();      //得到表的总列数
   LPCTSTR FieldName(int pos_zero_indexed);//根据索引，得到表的列名
   LPCTSTR FieldType(int pos_zero_indexed);//根据索引，得到表的列数据类型

   int ValueInt(int pos_zero_indexed);
   LPCTSTR ValueString(int pos_zero_indexed);//根据索引，得到表某列的值
