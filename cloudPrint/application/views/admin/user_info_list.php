...
<script type="text/javascript">
	$(document).ready(function(){
		/** 新增   **/
	    $("#addBtn").fancybox({
	    	'href'  : '<?php echo site_url('admin/business/user_add'); ?>',
	    	'width' : 520,
	        'height' : 520,
	        'type' : 'iframe',
	        ...
	        'onClosed' : function() { 
	        	window.location.href = '<?php echo site_url('admin/business/user_info'); ?>';
	        }
	    });
	...
	function del(ID){
        layer.confirm('删除后无法恢复，确定要删除么？', {
            ...
            $.ajax({
                url : "<?php echo site_url('admin/business/del_user'); ?>",
                data : {ID:ID},
                type : "POST",
                dataType : "json",
                async:true,
	...
	function jumpNormalPage(page){
		...
		$("#submitForm").attr("action", "<?php echo base_url(); ?>admin/business/get_user_info/" + page).submit();
	}
	...
	function jumpInputPage(totalPage){
		...
		if($("#jumpNumTxt").val() != '' ){
			...
			$("#submitForm").attr("action", "<?php echo base_url(); ?>admin/business/get_user_info/" + pageNum).submit();
		}else{
			// “跳转页数”为空
			art.dialog({icon:'error', title:'友情提示', drag:false, resize:false, content:'请输入合适的页数，\n自动为您跳到首页', ok:true,});
			$("#submitForm").attr("action", "get_user_info/" + 1).submit();
		}
	}
</script>
<style>
                        ...
                        <?php
                            for($i=0;$i<count($info)-1;$i++) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="<?php echo site_url('admin/business/user_edit'); ?>/<?php echo $info[$i]['userPhone'];?>" class="edit">编辑</a>
