...
<script type="text/javascript">
	$(document).ready(function(){
	    ...
	    $("a.edit").fancybox({
	        ...
	        'onClosed' : function() { 
	        	window.location.href = '<?php echo site_url('admin/business/order_manage'); ?>';
	        }
	    });
	});
	/** 用户角色   **/
...
// 				return;
	...
	function jumpNormalPage(page){
		...
		$("#submitForm").attr("action", "get_order_info/" + page).submit();
	}
	...
	function jumpInputPage(totalPage){
		...
		if($("#jumpNumTxt").val() != '' ){
			...
			$("#submitForm").attr("action", "get_order_info/" + pageNum).submit();
		}else{
			// “跳转页数”为空
			art.dialog({icon:'error', title:'友情提示', drag:false, resize:false, content:'请输入合适的页数，\n自动为您跳到首页', ok:true,});
			$("#submitForm").attr("action", "get_order_info/" + 1).submit();
		}
	}
</script>
<style>
                        ...
                        <?php
                            for($i=0;$i<count($info)-1;$i++) {
                                ...
                                <tr>
                                    ...
                                    <td><?php echo $info[$i]['price'];  ?></td>
                                    <td><?php echo $info[$i]['payStatus'] ? "已支付" : "未支付";
                                        ?></td>
                                    ...
                                    <td>
                                        ...
                                        <a href="<?php echo site_url('admin/business/down_file') ?>/<?php echo $info[$i]['orderId'];?>">下载</a>
